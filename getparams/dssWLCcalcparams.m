...
addParamValue(p,'cutoff',1e-4,@isnumeric);
addParamValue(p,'LMAX',-1,@(x) (isnumeric(x) && x <= 20 && x>=0));
...
if any(ismember('interpfile',p.UsingDefaults))
    ...
    if (any(ismember('alpha',p.UsingDefaults)) || length(p.Results.alpha)>1)
        ...
        klist = logspace(log10(1/del/100),log10(1/del*2),nk);
